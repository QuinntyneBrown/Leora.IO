...
namespace Leora.Commands.CQRS
{
    ...
    {
        public GenerateGetCommand(ILeoraJSONFileManager leoraJSONFileManager, ITemplateManager templateManager, IDotNetTemplateProcessor templateProcessor, INamingConventionConverter namingConventionConverter, IProjectManager projectManager, IFileWriter fileWriter, INamespaceManager namespaceManager)
        ...
        public int Run(string entityName, string namespacename, string directory, string name, string rootNamespace, string framework)
        {
            ...
            var templateCs = _templateManager.Get(FileType.CSharp, "CQRSGet", "Queries", _namingConventionConverter.Convert(NamingConvention.PascalCase, name), framework);
