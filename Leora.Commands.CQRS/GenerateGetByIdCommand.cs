...
namespace Leora.Commands.CQRS
{
    ...
    public class GenerateGetByIdCommand : Leora.Commands.CQRS.Core.BaseCommand<GenerateGetByIdOptions>, IGenerateGetByIdCommand
    {
        ...
        public override int Run(GenerateGetByIdOptions options)
        {
            ...
            {
                options.Name = $"Get{options.Entity}ById";
            }
        ...
        public int Run(string entityName, string namespacename, string directory, string name, string rootNamespace, string framework)
        {
            ...
            var templateCs = _templateManager.Get(FileType.CSharp, "CQRSGetById", "Queries", _namingConventionConverter.Convert(NamingConvention.PascalCase, name), framework);
